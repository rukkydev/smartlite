...
<main class="grid w-full grow grid-cols-1 place-items-center min-h-screen bg-purple-50 py-12 px-4">
    <div class="w-full max-w-xl p-4 sm:px-5">
        ...
        <div class="bg-white border border-purple-100 rounded-lg shadow-lg px-8 py-10">
            <h2 class="text-2xl font-semibold text-purple-800 mb-2">Enable Two-Factor Authentication</h2>
            <p class="text-purple-400 mb-6">Scan the QR code with your authenticator app.</p>
            <div class="flex justify-center mb-6">{!! auth()->user()->twoFactorQrCodeSvg() !!}</div>
            @if($errors->any())
            ...
            @endif
            <form method="POST" action="{{ route('two-factor.confirm') }}" x-data="{ loading: false }" @submit="loading = true" class="space-y-6 mt-5">
                @csrf
                <label class="block">
                    <span class="text-purple-800 block">Verification Code</span>
                    <span class="relative mt-1.5 flex">
                        <input
                            ...
                            type="text" name="code" id="code" required autocomplete="one-time-code" placeholder="Enter verification code" />
                        <span
                ...
                <button type="submit"
                    ...
                    :disabled="loading">
                    <span x-show="!loading" class="text-md">Confirm 2FA</span>
